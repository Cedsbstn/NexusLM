...
def before_tool(
    ...
    if tool.name == "send_meeting_invitation":
        # If the tool is send_meeting_invitation, we can check if the customer profile
        # is loaded in the state.
        if "customer_profile" not in tool_context.state:
            return "No customer profile loaded. Please load a customer profile before sending a meeting invitation."
